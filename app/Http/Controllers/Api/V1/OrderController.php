    ...
    public function place_order(Request $request)
    {
        ...
        if ($request->order_type !== 'parcel') {
         ...
         /*   if ($store->open == false) {
            ...
            $per_km_shipping_charge_car = (float)BusinessSetting::where(['key' => 'per_km_shipping_charge_car'])->first()->value;
            $minimum_shipping_charge_car = (float)BusinessSetting::where(['key' => 'minimum_shipping_charge_car'])->first()->value;
            
            $original_delivery_charge = 0;
            if($request->delivery_type == "Car Delivery"){
                $original_delivery_charge = ($request->distance * $per_km_shipping_charge_car > $minimum_shipping_charge_car) ? $request->distance * $per_km_shipping_charge_car : $minimum_shipping_charge_car;
                
            }else if($request->delivery_type == "Bike Delivery"){
        ...
        try {
            ...
            DB::commit();
            $order->cacnel_reason = "new"; //to send notification to vendors
            if($request->delivery_type != "Leapord"){
                 $order->notify = false;
            }else{
                $order->notify = true;
            }
            ...
            try{
                if($order->order_status == 'pending' && config('mail.status'))
                {
                    ...
                    Mail::to($store->email)->send(new PlaceOrder($order->id));
                }
            }catch (\Exception $ex) {
                info($ex);
            }
