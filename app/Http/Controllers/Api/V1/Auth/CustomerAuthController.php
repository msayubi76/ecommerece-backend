...
class CustomerAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($check_duplicate_ref) {
            return response()->json(['errors'=>['code'=>'ref_code','message'=>'Referral code clready used']]);
        } else {
    ...
    public function login(Request $request)
    {
        ...
        if (auth()->attempt($data)) {
          ...
          /*  if ($customer_verification && !auth()->user()->is_phone_verified && env('APP_MODE') != 'demo') {
            ...
            User::where('id',auth()->user()->id)->update(["is_phone_verified" => 1]);
            $user = auth()->user();
            ...
            return response()->json(['token' => $token, 'is_phone_verified' => 1], 200);
        } else {
            $errors = [];
