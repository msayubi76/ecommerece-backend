    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            if($request->weight){
                ...
                'height.required' => "Height of parcel is required for category other than food or leave complete section empty",
                'length.required' => "Height of parcel is required for category other than food or leave complete section empty",
                'width.required' => "Height of parcel is required for category other than food or leave complete section empty",
            ]);
        }
        ...
        $food->stock = $request->current_stock??0;
          //product weight
         if($request->weight){
             $v_weight = 100 * ( ($request->height * $request->width * $request->length)/5000);
             if($v_weight < $request->weight){
                   $item->weight = $request->weight;
             }else{
                  $item->weight = $v_weight;
             }
         }
        $food->save();
